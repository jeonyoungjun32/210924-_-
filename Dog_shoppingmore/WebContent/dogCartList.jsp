...
function checkAll(theForm) {/* theForm하면 전부 값을 가지고 있다 */
	if(theForm.remove.length == undefined){/* undefined = 정의가 되지않았다 */ //장바구니 항목이 1개일 때 length가 undefined
		theForm.remove.checked = theForm.allCheck.checked;//전부체크한 all체크가 trun이면  전부다 체크할라고 trun한다   여러개는배열(remove[i])로 들어간다 
...
function checkQty(kind, qty) {
	if(qty != 1){//현재 수량이 1이 아니면
		//장바구니 항목 수량 감소 요청을 함. ★★★★ 이때 kind값이 한글이면 한글이 깨지지 않도록 인코딩처리하여 한글파라미터로 전송
		//location.href="dogCartQtyDown.dog?kind="+encodeURIComponent(kind);  //서버측에 utf-8로 인코딩 됌!
		location.href="dogCartQtydown.dog?kind="+encodeURIComponent(kind);//utf-8로 인코딩됨
	}
}
</script>
<body>
		...
		<c:forEach var="cart" items="${cartList }" varStatus="status" > 
			<tr>
				...
				<td> <!--★★★★★★★★★    ${cart.kind }그냥 보내면 utf-8로 설정 안되어 있어서 찾을수가 없다 ★★★★★★★ 그래서 utf-8로 할라고 메서드 하나 만든다!!  -->
					...
					<a href="dogCartQtyUp.dog?kind=${cart.encodingKind }"><img src="images/up.jpg" id="upImage" border=0></a> <!-- 수량 1씩 증가할라면 qtyup을 요청한다 -->
					<br> 
					${cart.qty} <!-- 현재수량! -->
					<br>          
